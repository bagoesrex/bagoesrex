<h1 align="center">Hi, I'm Bagoes Rex 👋</h1>

<p align="center">
  Mobile developer • Full-stack developer • Android modder
</p>

<p align="center">
  <a href="https://t.me/bagoesrex">Telegram</a> ·
  <a href="https://instagram.com/bagoesrex">Instagram</a> ·
  <a href="mailto:bagoesrex@gmail.com">Email</a>
</p>

## About me

I enjoy building mobile and web applications, exploring Android customization, and learning new technologies. I primarily work with TypeScript.

```ts
type Proficiency = "intermediate" | "learning";

interface Laptop {
  brand: "Lenovo";
  model: string;
  processor: string;
  memory: string;
  graphics: string;
  role: string;
}

class Kyrex {
  readonly contact = {
    telegram: "t.me/bagoesrex",
    instagram: "instagram.com/bagoesrex",
    email: "bagoesrex@gmail.com",
  };

  readonly languages = ["Bahasa Indonesia", "English"] as const;

  readonly coding: Record<Proficiency, readonly string[]> = {
    intermediate: ["Kotlin", "PHP","Dart", "JavaScript", "TypeScript"],
    learning: ["C#", "Go"],
  };

  readonly specialties = [
    "Mobile development",
    "Full-stack development",
    "Android modding",
  ] as const;

  readonly tools = ["VS Code", "Android Studio"] as const;

  readonly laptops: Laptop[] = [
    {
      brand: "Lenovo",
      model: "LOQ 15IRX9",
      processor: "Intel Core i5 (13th Gen)",
      memory: "16 GB RAM",
      graphics: "NVIDIA GeForce RTX 3050",
      role: "Primary development laptop",
    },
    {
      brand: "Lenovo",
      model: "14ALC7",
      processor: "AMD Ryzen 3 5300U",
      memory: "16 GB RAM",
      graphics: "AMD Radeon RX Vega 6",
      role: "Secondary and portable development laptop",
    },
  ];
}
```

## Skills

<p align="center">
  <a href="https://skillicons.dev">
    <img src="https://skillicons.dev/icons?i=js,typescript,nodejs,react,dart,kotlin,nextjs,tailwindcss,php,flutter,vscode,androidstudio" alt="Technology skills" />
  </a>
</p>

<p align="center">
  <i>Always learning, always building.</i>
</p>
